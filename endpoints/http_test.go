...
func TestServerMuxGetMethods(t *testing.T) {
	...
	c.RegisterEndpoint("/test")
	assert.Equal(0, len(c.GetMethods()))
	RegisterEndpointMethod(c, "test", func(ctx context.Context, name string) (string, *rpc.Error) {
...
func TestHttpRequest(t *testing.T) {
	...
	RegisterServerMuxEndpointMethod(mux, "/hello", "hello", func(ctx context.Context, name string) (string, *rpc.Error) {
		return "Hello " + name, nil
	})
	go s.ListenAndServe()
	defer s.Close()
...
func TestHttpBatchRequest(t *testing.T) {
	...
	RegisterServerMuxEndpointMethod(mux, "/", "hello", func(ctx context.Context, name string) (string, *rpc.Error) {
		return "Hello " + name, nil
	})
	go s.ListenAndServe()
	defer s.Close()
